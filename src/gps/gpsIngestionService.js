 ...
 * Updates device_commands → status=acknowledged + stores device response text.
 * Also syncs device_state.relay_status when the response indicates relay state.
 */
export const handleCommandResponse = async ({ deviceId, parsed }) => {
  const { serverFlag, content } = parsed?.commandResponse || {};
  if (!deviceId || (!serverFlag && !content && !parsed?.commandResponse?.raw)) {
    return null;
  }

  try {
    const responseText = content || parsed?.commandResponse?.raw || '';
    const record = await acknowledgeDeviceCommandByFlag(deviceId, serverFlag, responseText);

    if (record) {
      logger.info(
        `Command ack: deviceId=${deviceId} id=${record.id} flag=${serverFlag} status=acknowledged response="${responseText}"`
      );
    } else {
      logger.info(`Command ack: no open command for deviceId=${deviceId} flag=${serverFlag}`);
    }

    // Sync relay from device response text when recognizable
    if (responseText) {
